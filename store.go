...
type Client interface {
	...
	ScriptExists(ctx context.Context, sha string) *libredis.BoolCmd
}
...
func (store *Store) preloadLuaScripts(ctx context.Context) error {
	...
	if atomic.LoadUint32(&store.luaLoaded) == 0 {
		incrExits, err := store.client.ScriptExists(ctx, store.getLuaIncrSHA()).Result()

		if err != nil {
			return errors.Wrap(err, `failed to check "incr" lua script`)
		}

		peekExits, err := store.client.ScriptExists(ctx, store.getLuaPeekSHA()).Result()

		if err != nil {
			return errors.Wrap(err, `failed to check "peek" lua script`)
		}

		if incrExits && peekExits {
